...
Answer: [ONLY write a single letter - either a, b, c, or d - nothing else]
Reasoning: [Write your explanation here]

Do not include any other text or formatting. Start your response with "Answer:" followed by a single letter.

Based on the following information and your memory of previous questions:
{scrambled_elements}
...
{memory_str}""",
    ...
    def answer(self, mcq, image_path, scrambled_elements):
        ...
        if response.status_code == 200:
            ...
            print(response.text)
            return "Invalid", "Error occurred", response_text
        
        # Extract answer and reasoning using regex with improved patterns
        answer_pattern = r'Answer:\s*([abcdABCD])'
        ...
        if answer_match:
            mcq_answer = answer_match.group(1).lower()
            # Additional validation to ensure it's a valid option
            if mcq_answer not in ['a', 'b', 'c', 'd']:
                print(f"Warning: Invalid answer format detected in response: {response_text}")
                mcq_answer = "Invalid"
        else:
            print(f"Warning: No answer found in response: {response_text}")
            mcq_answer = "Invalid"
        
        reasoning = reasoning_match.group(1).strip() if reasoning_match else "No reasoning provided"
        
        # Debug logging
        if mcq_answer == "Invalid":
            print(f"Full response text:\n{response_text}")
        
        return mcq_answer, reasoning, response_text

    def update_memory(self, question, image_path, answer, reasoning, correct_answer):
        result = "correct" if answer.upper() == correct_answer.upper() else "incorrect"
        
        self.cursor.execute('''
            INSERT INTO questions (question, image_path, agent_id, answer, reasoning, correct_answer, result)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (question, image_path, self.agent_id, answer, reasoning, correct_answer, result))
        
        self.db_conn.commit()

    def get_memory(self):
        self.cursor.execute('''
            SELECT question, answer, reasoning, correct_answer, result
            FROM questions
            WHERE agent_id = ?
        ...
        for row in self.cursor.fetchall():
            question, answer, reasoning, correct_answer, result = row
...
def get_all_responses(agents, elements, mcq, image_path, correct_answer):
    ...
    for agent in agents:
        ...
        mcq_answer, reasoning, full_response = agent.answer(mcq, image_path, scrambled_str)
        
        is_correct = mcq_answer.upper() == correct_answer.upper()
        agent.update_memory(mcq, image_path, mcq_answer, reasoning, correct_answer)
    ...
    for resp in responses:
        ...
        print(f"Correct: {'✓' if resp['correct'] else '✗'}")
...
def export_results_to_json(question_accuracies, output_file="results.json"):
    """Export the accuracy results to a JSON file."""
    results = {
        f"question_{i+1}": accuracies 
...
def print_final_statistics(question_accuracies):
    """Print final statistics for all questions."""
    print("\nFinal Statistics:")
    print("-" * 40)
    ...
    for i, accuracies in question_accuracies.items():
        ...
        print(f"Trend: {' -> '.join(f'{acc:.2f}' for acc in accuracies)}")
    ...
    try:
        while True:
            ...
            for i, question_data in enumerate(questions):
                ...
                print(f"\nQuestion {i+1} - Accuracy: {accuracy:.2f}")
            ...
            # Export current results after each iteration
            export_results_to_json(question_accuracies, results_file)
        ...
        print_final_statistics(question_accuracies)
